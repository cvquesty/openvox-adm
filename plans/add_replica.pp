...
plan openvoxadm::add_replica (
  ...
  run_task('openvoxadm::install_packages', $replica_target, version => $version)

  # Bootstrap cert on replica
  run_command('puppet ssl bootstrap --waitforcert 60', $replica_target)

  $primary_name = openvoxadm::certname($primary_target)

  # Configure replica as secondary server
  run_command("puppet config set server ${primary_name} --section main", $replica_target)
  run_command("puppet config set ca_server ${primary_name} --section main", $replica_target)
  ...
  # If replica has its own PostgreSQL — only resolve targets when defined
  if $replica_postgresql_host {
    $replica_postgresql_target = get_targets($replica_postgresql_host, 1)
    run_task('openvoxadm::install_packages', $replica_postgresql_target, version => $version)
